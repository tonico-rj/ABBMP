        ...
        <div class="box-body">
            <table class='table table-striped' id="tbl_">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>{!! trans("mensagens.forca") !!}</th>
                    </tr>
                </thead>
                <tbody>
                @foreach ($registros as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'forca.destroy', 'forca' => $reg->forca_id]
                                , 'method' =>'DELETE'
                                , 'id' => "delete-form-{$reg->forca_id}"
                                , 'style' => 'display:none'
                                ]) !!}
                        {!! Form::close() !!}
                        <td>{{$reg->forca_id}}</td>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('forca/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('mensagens.exc_forca') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
