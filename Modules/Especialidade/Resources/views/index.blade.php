...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4>{!! trans('mensagens.especialidade') !!}</h4>
        </div>
        <div class="box-body">
            <table class='table table-striped' id="tbl_">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>{!! trans("mensagens.especialidade") !!}</th>
                    </tr>
                </thead>
                <tbody>
                @foreach ($registros as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'especialidade.destroy', 'especialidade' => $reg->especialidade_id]
                                , 'method' =>'DELETE'
                                , 'id' => "delete-form-{$reg->especialidade_id}"
                                , 'style' => 'display:none'
                                ]) !!}
                        {!! Form::close() !!}
                        <td>{{$reg->especialidade_id}}</td>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('especialidade/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('especialidade')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
